...
def _int8_delta(metric, size, arm="base"):
    """FP32->INT8 delta for the deployed point from int8_validation.csv.

    Prefers the NNTool (deployed-quantiser) delta row over the ONNX-Runtime proxy row,
    and seed 0 over other seeds; reruns append, so the LAST matching row wins (mirrors
    scripts/int8_summary.py). None when the CSV or the row is absent.
    """
    best = {}
    for r in _read("int8_validation.csv"):
        if (_col(r, "arm"), _col(r, "size"), _col(r, "precision")) != \
                (arm, str(size), "delta(int8-fp32)"):
            continue
        f = "nntool" if _col(r, "provenance").lower().startswith("nntool") else "ort"
        seed_pref = 0 if _col(r, "seed") == "0" else 1
        if f not in best or seed_pref <= best[f][0]:
            best[f] = (seed_pref, r)
    row = (best.get("nntool") or best.get("ort") or (None, None))[1]
    if row is None:
        return None
    try:
        return float(_col(row, metric))
    except (TypeError, ValueError):
        return None


def _fmt_delta(v, fmt):
    """Signed delta body, or the placeholder when the measurement does not exist yet."""
    return PLACEHOLDER if v is None else fmt.format(v)
...
def build_values():
    ...
    return [
        ...
        # FP32->INT8 quantisation deltas at the deployed point (NNTool SQ8/NE16 row
        # preferred, ONNX-Runtime proxy fallback) -- the reviewer-requested "accuracy
        # before/after INT8". map/F1 in points, MAE/bias in percentage points.
        ("valIntEightDeltaMap",  _fmt_delta(_int8_delta("map50", size), "{:+.3f}")),
        ("valIntEightDeltaFOne", _fmt_delta(_int8_delta("micro_f1", size), "{:+.3f}")),
        ("valIntEightDeltaMae",  _fmt_delta(_int8_delta("count_mae_pct", size), "{:+.1f}")),
        ("valIntEightDeltaBias", _fmt_delta(_int8_delta("net_bias_pct", size), "{:+.1f}")),
    ]
