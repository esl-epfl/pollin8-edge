...
# INT8 validation (50_int8_validate.sbatch) needs onnx + the CPU-ONLY onnxruntime. The default
# `onnxruntime` wheel is now a CUDA-13 build that fails to import on the nodes (libcudart.so.13);
# 1.20.x is CPU-only and numpy-2.x compatible. Install here (login node) so jobs never pip on compute.
pip install onnx "onnxruntime==1.20.1"
