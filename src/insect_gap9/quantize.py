...
def _have_nntool() -> bool:
    try:
        # Probe nntool.api, not bare `import nntool`: the PyPI package named "nntool" is an
        # unrelated project that would select the real path here and crash inside it.
        from nntool.api import NNGraph  # noqa: F401  (ships with the GAP SDK)
        return True
    except Exception:
        return False
