...
> **Accuracy flow exports FP32, not `--int8`.** For the *latency/cycle* flow above an int8
> tflite is fine (nntool re-quantises from its own statistics either way). For the *accuracy*
> validation (`docs/training_kuma.md` §6b, `validate_int8_nntool.py`) export **FP32 tflite**
> (`--include tflite --imgsz 320`, no `--int8`): chaining TF's quantiser in front of NNTool's
> would contaminate the measured FP32→INT8 delta with a second, unrelated quantiser. §6b's
> full-decode graph is also the same artifact this autotiler flow can consume (the SENSEI demo
> ships full-decode tflites: `ROWS` in the CMake = total anchors), so the truncated export here
> is an optimisation for the cycle count, not a requirement.
